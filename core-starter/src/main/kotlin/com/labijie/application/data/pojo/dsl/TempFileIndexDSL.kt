...
import com.labijie.application.`data`.TempFileIndexTable.sizeInBytes
...
public object TempFileIndexDSL {
  ...
  public fun parseRow(raw: ResultRow): TempFileIndex {
    ...
    plain.sizeInBytes = raw[sizeInBytes]
  ...
  public fun parseRowSelective(row: ResultRow): TempFileIndex {
    ...
    if(row.hasValue(sizeInBytes)) {
      plain.sizeInBytes = row[sizeInBytes]
    }
  ...
  private fun <T> TempFileIndex.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    TempFileIndexTable.sizeInBytes -> this.sizeInBytes.toString()
    ...
    else->throw IllegalArgumentException("""Can't converter value of TempFileIndex::${column.name} to string.""")
  }
  ...
  private fun <T> parseColumnValue(valueString: String, column: Column<T>): T {
    val value = when(column) {
      ...
      TempFileIndexTable.sizeInBytes ->valueString.toLong()
  ...
  public fun <T> TempFileIndex.getColumnValue(column: Column<T>): T = when(column) {
    ...
    TempFileIndexTable.sizeInBytes->this.sizeInBytes as T
  ...
  public fun assign(
    ...
    if((list == null || list.contains(sizeInBytes)) && !ignore.contains(sizeInBytes))
      builder[sizeInBytes] = raw.sizeInBytes
  ...
  public fun Iterable<ResultRow>.toTempFileIndexList(vararg selective: Column<*>): List<TempFileIndex> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: TempFileIndex, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: TempFileIndex, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)

  public fun TempFileIndexTable.insert(raw: TempFileIndex): InsertStatement<Number> = insert {
    assign(it, raw)
  }

  public fun TempFileIndexTable.insertIgnore(raw: TempFileIndex): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun TempFileIndexTable.updateByPrimaryKey(raw: TempFileIndex, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(id)) {
    TempFileIndexTable.id.eq(raw.id)
  }

  public fun TempFileIndexTable.updateByPrimaryKey(id: Long, builder: TempFileIndexTable.(UpdateStatement) -> Unit): Int = update({ TempFileIndexTable.id.eq(id) }, body = builder)

  public fun TempFileIndexTable.deleteByPrimaryKey(id: Long): Int = deleteWhere {
    TempFileIndexTable.id.eq(id)
  }

  public fun TempFileIndexTable.selectByPrimaryKey(id: Long, vararg selective: Column<*>): TempFileIndex? {
  ...
  public fun TempFileIndexTable.selectByPrimaryKeys(ids: Iterable<Long>, vararg selective: Column<*>): List<TempFileIndex> {
    val query = selectSlice(*selective).andWhere {
  ...
  public fun TempFileIndexTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<TempFileIndex> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toTempFileIndexList(*selective)
  }

  public fun TempFileIndexTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): TempFileIndex? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toTempFileIndex(*selective)
  }
  ...
  public fun TempFileIndexTable.selectForwardByPrimaryKey(
    ...
    val list = sorted.limit(pageSize + 1).toTempFileIndexList(*selective.toTypedArray()).toMutableList()
