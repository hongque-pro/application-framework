...
import com.labijie.application.`data`.FileIndexTable.sizeInBytes
...
public object FileIndexDSL {
  ...
  private fun <T> FileIndex.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    FileIndexTable.sizeInBytes -> this.sizeInBytes.toString()

    FileIndexTable.entityId -> this.entityId.toString()

    FileIndexTable.id -> this.id.toString()

    else->throw IllegalArgumentException("""Can't converter value of FileIndex::${column.name} to string.""")
  }
  ...
  private fun <T> parseColumnValue(valueString: String, column: Column<T>): T {
    val value = when(column) {
      ...
      FileIndexTable.sizeInBytes ->valueString.toLong()
  ...
  public fun <T> FileIndex.getColumnValue(column: Column<T>): T = when(column) {
    ...
    FileIndexTable.sizeInBytes->this.sizeInBytes as T
  ...
  public fun Iterable<ResultRow>.toFileIndexList(vararg selective: Column<*>): List<FileIndex> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: FileIndex, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: FileIndex, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)
  ...
  public fun FileIndexTable.updateByPrimaryKey(raw: FileIndex, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(id)) {
    FileIndexTable.id.eq(raw.id)
  }

  public fun FileIndexTable.updateByPrimaryKey(id: Long, builder: FileIndexTable.(UpdateStatement) -> Unit): Int = update({ FileIndexTable.id.eq(id) }, body = builder)
  ...
  public fun FileIndexTable.selectByPrimaryKeys(ids: Iterable<Long>, vararg selective: Column<*>): List<FileIndex> {
    val query = selectSlice(*selective).andWhere {
  ...
  public fun FileIndexTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<FileIndex> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toFileIndexList(*selective)
  }

  public fun FileIndexTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): FileIndex? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toFileIndex(*selective)
  }
