...
import com.labijie.application.identity.data.mapper.RoleClaimMapper
import org.mybatis.dynamic.sql.SqlBuilder.isEqualTo
import org.mybatis.dynamic.sql.util.kotlin.*
import org.mybatis.dynamic.sql.util.kotlin.mybatis3.*

fun RoleClaimMapper.count(completer: CountCompleter) =
    countFrom(this::count, DynamicTableSupport.getTable(RoleClaim), completer)

fun RoleClaimMapper.delete(completer: DeleteCompleter) =
    deleteFrom(this::delete, DynamicTableSupport.getTable(RoleClaim), completer)
...
fun RoleClaimMapper.insert(record: RoleClaimRecord) =
    insert(this::insert, record, DynamicTableSupport.getTable(RoleClaim)) {
...
fun RoleClaimMapper.insertMultiple(records: Collection<RoleClaimRecord>) =
    insertMultiple(this::insertMultiple, records, DynamicTableSupport.getTable(RoleClaim)) {
        map(id).toProperty("id")
        map(claimType).toProperty("claimType")
        map(claimValue).toProperty("claimValue")
        map(roleId).toProperty("roleId")
...
fun RoleClaimMapper.selectOne(completer: SelectCompleter) =
    selectOne(this::selectOne,
        columnList, DynamicTableSupport.getTable(RoleClaim), completer)
...
fun RoleClaimMapper.update(completer: UpdateCompleter) =
    update(this::update, DynamicTableSupport.getTable(RoleClaim), completer)
