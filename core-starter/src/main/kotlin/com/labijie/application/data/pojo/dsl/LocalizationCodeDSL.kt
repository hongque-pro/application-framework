...
public object LocalizationCodeDSL {
  ...
  public fun <T> LocalizationCodeTable.getColumnType(column: Column<T>): KClass<*> = when(column) {
    code->String::class
    else->throw IllegalArgumentException("""Unknown column <${column.name}> for 'LocalizationCode'""")
  }
  ...
  private fun <T> LocalizationCode.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    else->throw IllegalArgumentException("""Can't converter value of LocalizationCode::${column.name} to string.""")
  }
  ...
  public fun Iterable<ResultRow>.toLocalizationCodeList(vararg selective: Column<*>): List<LocalizationCode> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: LocalizationCode, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: LocalizationCode, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)

  public fun LocalizationCodeTable.insert(raw: LocalizationCode): InsertStatement<Number> = insert {
    assign(it, raw)
  }

  public fun LocalizationCodeTable.insertIgnore(raw: LocalizationCode): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun LocalizationCodeTable.batchUpsert(
    ...
    val rows =  batchUpsert(data = list, keys = arrayOf(code), onUpdate = onUpdate, onUpdateExclude = onUpdateExclude, where = where, shouldReturnGeneratedValues = shouldReturnGeneratedValues) {
  ...
  public fun LocalizationCodeTable.updateByPrimaryKey(raw: LocalizationCode, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(code)) {
    LocalizationCodeTable.code.eq(raw.code)
  }

  public fun LocalizationCodeTable.updateByPrimaryKey(code: String, builder: LocalizationCodeTable.(UpdateStatement) -> Unit): Int = update({ LocalizationCodeTable.code.eq(code) }, body = builder)

  public fun LocalizationCodeTable.deleteByPrimaryKey(code: String): Int = deleteWhere {
    LocalizationCodeTable.code.eq(code)
  }

  public fun LocalizationCodeTable.selectByPrimaryKey(code: String, vararg selective: Column<*>): LocalizationCode? {
  ...
  public fun LocalizationCodeTable.selectByPrimaryKeys(ids: Iterable<String>, vararg selective: Column<*>): List<LocalizationCode> {
    val query = selectSlice(*selective).andWhere {
  ...
  public fun LocalizationCodeTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<LocalizationCode> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toLocalizationCodeList(*selective)
  }

  public fun LocalizationCodeTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): LocalizationCode? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toLocalizationCode(*selective)
  }
  ...
  public fun LocalizationCodeTable.selectForwardByPrimaryKey(
    ...
    val list = sorted.limit(pageSize + 1).toLocalizationCodeList(*selective.toTypedArray()).toMutableList()
  ...
  public fun <T : Comparable<T>> LocalizationCodeTable.selectForward(
    ...
    val lastId = if(kp != null && kp.size > 1 && kp[1].isNotBlank()) parseColumnValue(kp[1], code) else null
    val query = selectSlice(*selective.toTypedArray())
    offsetKey?.let {
      when(order) {
        SortOrder.DESC, SortOrder.DESC_NULLS_FIRST, SortOrder.DESC_NULLS_LAST->
    ...
    if(lastId != null && offsetKey != null) {
      when(order) {
        SortOrder.DESC, SortOrder.DESC_NULLS_FIRST, SortOrder.DESC_NULLS_LAST->
        query.orWhere { sortColumn.eq(offsetKey) and code.less(lastId) }
        else-> query.orWhere { sortColumn.eq(offsetKey) and code.greater(lastId) }
      }
    }
    `where`?.invoke(query)
  ...
  public fun LocalizationCodeTable.replace(raw: LocalizationCode): ReplaceStatement<Long> = replace {
    assign(it, raw)
  }
}
