...
import com.labijie.application.auth.data.mapper.RoleDynamicSqlSupport.Role
import com.labijie.application.auth.data.mapper.RoleDynamicSqlSupport.Role.concurrencyStamp
import com.labijie.application.auth.data.mapper.RoleDynamicSqlSupport.Role.id
import com.labijie.application.auth.data.mapper.RoleDynamicSqlSupport.Role.name
...
fun RoleMapper.count(completer: CountCompleter) =
    countFrom(this::count, AuthDynamicTableSupport.getTable(Role), completer)

fun RoleMapper.delete(completer: DeleteCompleter) =
    deleteFrom(this::delete, AuthDynamicTableSupport.getTable(Role), completer)
...
fun RoleMapper.insert(record: RoleRecord) =
    insert(this::insert, record, AuthDynamicTableSupport.getTable(Role)) {
...
fun RoleMapper.insertMultiple(records: Collection<RoleRecord>) =
    insertMultiple(this::insertMultiple, records, AuthDynamicTableSupport.getTable(Role)) {
        map(id).toProperty("id")
        map(concurrencyStamp).toProperty("concurrencyStamp")
        map(name).toProperty("name")
    }
...
fun RoleMapper.selectOne(completer: SelectCompleter) =
    selectOne(this::selectOne,
        columnList, AuthDynamicTableSupport.getTable(Role), completer)
...
fun RoleMapper.update(completer: UpdateCompleter) =
    update(this::update, AuthDynamicTableSupport.getTable(Role), completer)
