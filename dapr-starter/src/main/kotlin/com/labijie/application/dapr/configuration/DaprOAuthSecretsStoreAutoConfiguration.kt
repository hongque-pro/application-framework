...
import com.labijie.application.dapr.components.DaprDbEnvironmentPostProcessor
...
class DaprOAuthSecretsStoreAutoConfiguration {
    ...
    @Configuration(proxyBeanMethods = false)
    @ConditionalOnProperty(prefix = "application.dapr.secrets-store", name = ["oauth2-server-private-key", "oauth2-server-public-key"], matchIfMissing = false)
    ...
    class DaprOAuth2ServerAutoConfiguration {
        ...
        fun daprOAuth2ServerSecretsStore(daprClient: DaprClient, properties: DaprProperties) : DaprOAuth2ServerSecretsStore {
            logger.info("OAuth2 server secret from dapr secrets store.")
    ...
    @ConditionalOnProperty(prefix = "application.dapr.secrets-store", name = ["resource-server-public-key"], matchIfMissing = false)
    fun daprResourceServerSecretsStore(daprClient: DaprClient, properties: DaprProperties) : DaprResourceServerSecretsStore {
        logger.info("OAuth2 resource server secret from dapr secrets store.")
