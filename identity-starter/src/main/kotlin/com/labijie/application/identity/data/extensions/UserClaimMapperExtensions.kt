...
import com.labijie.application.identity.data.mapper.UserClaimMapper
...
fun UserClaimMapper.count(completer: CountCompleter) =
    countFrom(this::count, DynamicTableSupport.getTable(UserClaim), completer)

fun UserClaimMapper.delete(completer: DeleteCompleter) =
    deleteFrom(this::delete, DynamicTableSupport.getTable(UserClaim), completer)
...
fun UserClaimMapper.insert(record: UserClaimRecord) =
    insert(this::insert, record, DynamicTableSupport.getTable(UserClaim)) {
...
fun UserClaimMapper.insertMultiple(records: Collection<UserClaimRecord>) =
    insertMultiple(this::insertMultiple, records, DynamicTableSupport.getTable(UserClaim)) {
        map(id).toProperty("id")
        map(claimType).toProperty("claimType")
        map(claimValue).toProperty("claimValue")
        map(userId).toProperty("userId")
...
fun UserClaimMapper.selectOne(completer: SelectCompleter) =
    selectOne(this::selectOne,
        columnList, DynamicTableSupport.getTable(UserClaim), completer)
...
fun UserClaimMapper.update(completer: UpdateCompleter) =
    update(this::update, DynamicTableSupport.getTable(UserClaim), completer)
