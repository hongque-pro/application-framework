...
import com.labijie.application.identity.data.mapper.UserLoginMapper
...
fun UserLoginMapper.count(completer: CountCompleter) =
    countFrom(this::count, DynamicTableSupport.getTable(UserLogin), completer)

fun UserLoginMapper.delete(completer: DeleteCompleter) =
    deleteFrom(this::delete, DynamicTableSupport.getTable(UserLogin), completer)
...
fun UserLoginMapper.insert(record: UserLoginRecord) =
    insert(this::insert, record, DynamicTableSupport.getTable(UserLogin)) {
...
fun UserLoginMapper.insertMultiple(records: Collection<UserLoginRecord>) =
    insertMultiple(this::insertMultiple, records, DynamicTableSupport.getTable(UserLogin)) {
        map(loginProvider).toProperty("loginProvider")
        map(providerKey).toProperty("providerKey")
        map(userId).toProperty("userId")
        map(providerDisplayName).toProperty("providerDisplayName")
...
fun UserLoginMapper.selectOne(completer: SelectCompleter) =
    selectOne(this::selectOne,
        columnList, DynamicTableSupport.getTable(UserLogin), completer)
...
fun UserLoginMapper.update(completer: UpdateCompleter) =
    update(this::update, DynamicTableSupport.getTable(UserLogin), completer)
