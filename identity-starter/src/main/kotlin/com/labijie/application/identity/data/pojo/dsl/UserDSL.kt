...
public object UserDSL {
  ...
  private fun <T> parseColumnValue(valueString: String, column: Column<T>): T {
    val value = when(column) {
      ...
      UserTable.userType ->valueString.toByteOrNull()
  ...
  public fun UpdateBuilder<*>.setValue(raw: User, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: User, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)
  ...
  public fun UserTable.updateByPrimaryKey(raw: User, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(id)) {
    UserTable.id.eq(raw.id)
  }

  public fun UserTable.updateByPrimaryKey(id: Long, builder: UserTable.(UpdateStatement) -> Unit): Int = update({ UserTable.id.eq(id) }, body = builder)
  ...
  public fun UserTable.selectByPrimaryKeys(ids: Iterable<Long>, vararg selective: Column<*>): List<User> {
    val query = selectSlice(*selective).andWhere {
  ...
  public fun UserTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<User> {
