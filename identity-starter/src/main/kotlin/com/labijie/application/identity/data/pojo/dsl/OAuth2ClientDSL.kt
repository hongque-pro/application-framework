...
public object OAuth2ClientDSL {
  ...
  private fun <T> OAuth2Client.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    else->throw IllegalArgumentException("""Can't converter value of OAuth2Client::${column.name} to string.""")
  }
  ...
  public fun assign(
    ...
    if((list == null || list.contains(authorizedGrantTypes)) && !ignore.contains(authorizedGrantTypes))
    ...
    if((list == null || list.contains(additionalInformation)) && !ignore.contains(additionalInformation))
    ...
    if((list == null || list.contains(accessTokenLiveSeconds)) && !ignore.contains(accessTokenLiveSeconds))
      builder[accessTokenLiveSeconds] = raw.accessTokenLiveSeconds
    if((list == null || list.contains(refreshTokenLiveSeconds)) && !ignore.contains(refreshTokenLiveSeconds))
      builder[refreshTokenLiveSeconds] = raw.refreshTokenLiveSeconds
    if((list == null || list.contains(authorizationCodeLiveSeconds)) && !ignore.contains(authorizationCodeLiveSeconds))
      builder[authorizationCodeLiveSeconds] = raw.authorizationCodeLiveSeconds
    if((list == null || list.contains(deviceCodeLiveSeconds)) && !ignore.contains(deviceCodeLiveSeconds))
  ...
  public fun Iterable<ResultRow>.toOAuth2ClientList(vararg selective: Column<*>): List<OAuth2Client> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: OAuth2Client, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: OAuth2Client, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)

  public fun OAuth2ClientTable.insert(raw: OAuth2Client): InsertStatement<Number> = insert {
    assign(it, raw)
  }

  public fun OAuth2ClientTable.insertIgnore(raw: OAuth2Client): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun OAuth2ClientTable.batchUpsert(
    ...
    val rows =  batchUpsert(data = list, keys = arrayOf(), onUpdate = onUpdate, onUpdateExclude = onUpdateExclude, where = where, shouldReturnGeneratedValues = shouldReturnGeneratedValues) {
  ...
  public fun OAuth2ClientTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<OAuth2Client> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toOAuth2ClientList(*selective)
  }

  public fun OAuth2ClientTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): OAuth2Client? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
