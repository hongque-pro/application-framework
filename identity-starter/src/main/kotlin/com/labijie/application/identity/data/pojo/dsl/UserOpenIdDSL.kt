...
public object UserOpenIdDSL {
  ...
  private fun <T> UserOpenId.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    else->throw IllegalArgumentException("""Can't converter value of UserOpenId::${column.name} to string.""")
  }
  ...
  public fun Iterable<ResultRow>.toUserOpenIdList(vararg selective: Column<*>): List<UserOpenId> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: UserOpenId, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: UserOpenId, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)
  ...
  public fun UserOpenIdTable.batchUpsert(
    ...
    val rows =  batchUpsert(data = list, keys = arrayOf(userId, appId, loginProvider), onUpdate = onUpdate, onUpdateExclude = onUpdateExclude, where = where, shouldReturnGeneratedValues = shouldReturnGeneratedValues) {
  ...
  public fun UserOpenIdTable.updateByPrimaryKey(raw: UserOpenId, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(userId, appId, loginProvider)) {
    UserOpenIdTable.userId.eq(raw.userId) and UserOpenIdTable.appId.eq(raw.appId) and UserOpenIdTable.loginProvider.eq(raw.loginProvider)
  }
  ...
  ): Int = update({ UserOpenIdTable.userId.eq(userId) and UserOpenIdTable.appId.eq(appId) and UserOpenIdTable.loginProvider.eq(loginProvider) }, body = builder)
  ...
  public fun UserOpenIdTable.deleteByPrimaryKey(
    ...
    UserOpenIdTable.userId.eq(userId) and UserOpenIdTable.appId.eq(appId) and UserOpenIdTable.loginProvider.eq(loginProvider)
  }
  ...
  public fun UserOpenIdTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<UserOpenId> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toUserOpenIdList(*selective)
  }

  public fun UserOpenIdTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): UserOpenId? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toUserOpenId(*selective)
  }
