...
import com.labijie.application.auth.data.UserTokenRecord
import com.labijie.application.auth.data.mapper.UserRoleDynamicSqlSupport
import com.labijie.application.auth.data.mapper.UserTokenDynamicSqlSupport.UserToken
import com.labijie.application.auth.data.mapper.UserTokenDynamicSqlSupport.UserToken.loginProvider
import com.labijie.application.auth.data.mapper.UserTokenDynamicSqlSupport.UserToken.name
import com.labijie.application.auth.data.mapper.UserTokenDynamicSqlSupport.UserToken.token
import com.labijie.application.auth.data.mapper.UserTokenDynamicSqlSupport.UserToken.userId
import com.labijie.application.auth.data.mapper.UserTokenMapper
import org.mybatis.dynamic.sql.SqlBuilder.isEqualTo
import org.mybatis.dynamic.sql.util.kotlin.*
import org.mybatis.dynamic.sql.util.kotlin.mybatis3.*

fun UserTokenMapper.count(completer: CountCompleter) =
    countFrom(this::count, AuthDynamicTableSupport.getTable(UserToken), completer)

fun UserTokenMapper.delete(completer: DeleteCompleter) =
    deleteFrom(this::delete, AuthDynamicTableSupport.getTable(UserToken), completer)
...
fun UserTokenMapper.insert(record: UserTokenRecord) =
    insert(this::insert, record, AuthDynamicTableSupport.getTable(UserToken)) {
...
fun UserTokenMapper.insertMultiple(records: Collection<UserTokenRecord>) =
    insertMultiple(this::insertMultiple, records, AuthDynamicTableSupport.getTable(UserToken)) {
        map(userId).toProperty("userId")
        map(loginProvider).toProperty("loginProvider")
        map(token).toProperty("token")
        map(name).toProperty("name")
...
fun UserTokenMapper.selectOne(completer: SelectCompleter) =
    selectOne(this::selectOne,
        columnList, AuthDynamicTableSupport.getTable(UserToken), completer)
...
fun UserTokenMapper.update(completer: UpdateCompleter) =
    update(this::update, AuthDynamicTableSupport.getTable(UserToken), completer)
