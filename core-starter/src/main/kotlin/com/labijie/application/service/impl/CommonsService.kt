...
class CommonsService(
    ...
    private val areaMapper: RegionAreaMapper
) : ICommonService {
    ...
    private fun getRegionData(): RegionData {
        return cacheManager.getOrSet(CoreCacheKeys.REGION_CACHE_DATA, "REGION_DATA", Duration.ofHours(1)) {
            RegionData(
                provinces = provinceMapper.selectByExample(
                    RegionProvinceExample().apply {
                    this.orderByClause = RegionProvince.Column.id.asc()
                }),
                cities = cityMapper.selectByExample(
                    RegionCityExample().apply {
                    this.orderByClause = RegionCity.Column.id.asc()
                }),
                areas = areaMapper.selectByExample(
                    RegionAreaExample().apply {
